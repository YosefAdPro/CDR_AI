...
//			$csv_line[9]	= $row['lastdata'];
			$csv_line[3]	= $row['duration'];
			$csv_line[4]	= $row['billsec'];
			$csv_line[5]	= $row['disposition'];
...
if ( isset($_REQUEST['need_html']) && $_REQUEST['need_html'] == 'true' ) {
	...
	if ( isset($tot_calls_raw) && $tot_calls_raw ) {
		...
		try {
			...
			$filtered_count = 0;
			# Удаление дублирующихся רשומות в Asterisk 13
...
switch ($group) {
	case "disposition_by_day":
		$graph_col_title = 'סטטוס לפי ימים';
	...
	case "disposition_by_hour":
		$graph_col_title = 'סטטוס לפי שעה';
		$group_by_field_php = array( '%Y-%m-%d %H / ', 20, '' );
		$group_by_field = "CONCAT(DATE_FORMAT(calldate, '$group_by_field_php[0]'),disposition)";
	break;
	case "disposition":
		$graph_col_title = 'סטטוס';
	break;
	case "dcontext":
		$graph_col_title = 'תחביר';
	break;
	case "accountcode":
		$graph_col_title = 'קוד חשבון';
	break;
	case "dst":
		$graph_col_title = 'מחוייג';
		$graph_col_title = 'מחוייג';
	break;
	case "did":
		$graph_col_title = 'DID';
	break;
	case "src":
		$graph_col_title = 'מקור השיחה';
	break;
	case "clid":
		$graph_col_title = 'CallerID';
	break;
	case "userfield":
		$graph_col_title = 'פרמטר משתנה';
	break;
	case "hour":
